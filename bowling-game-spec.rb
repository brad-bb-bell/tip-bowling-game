...
RSpec.describe BowlingGame do
  ...
  describe "#my_test_method" do
    ...
    it "scores a game with an 'X' in the tenth frame" do
      endgame = [1, 5, 4, 4, 8, "/", 5, 3, "X", "X", 2, 3, 8, "/", 7, 1, "X", 5, 4]
      subject = described_class.new

      expect(subject.score(endgame)).to eq(123)
    end
    ...
    it "scores a perfect game" do
      ...
      expect(subject.score(perfect)).to eq(300)
    end

    it "scores a near perfect game" do
      near_perfect = ["X", "X", "X", "X", "X", "X", "X", "X", "X", "X", "X", 9]
      subject = described_class.new

      expect(subject.score(near_perfect)).to eq(299)
    end

    it "scores another near perfect game" do
      another_near_perfect = ["X", "X", "X", "X", "X", "X", "X", "X", "X", "X", 8, 1]
      subject = described_class.new

      expect(subject.score(another_near_perfect)).to eq(287)
    end
  end
end
